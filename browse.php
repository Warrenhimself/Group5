    ...
    <div class="col-md-3 pr-0">
      <div class="form-inline">
        ...
        <select class="form-control" id="order_by" name="order_by">
          <?php $order_by = $_GET['order_by'] ?? 'pricelow'; ?>
...
$curr_page = (isset($_GET['page']) && ctype_digit($_GET['page'])) ? (int)$_GET['page'] : 1;
...
if ($num_results == 0) {
  ...
  while ($row = $list_result->fetch_assoc()) {
    ...
    if ($now < $start_date) {
      ...
      $status_text = $num_bids . " bid" . ($num_bids == 1 ? "" : "s");
    }
...
if ($curr_page > 1) {
  echo '<li class="page-item">
          <a class="page-link" href="browse.php?' . $querystring . 'page=' . ($curr_page - 1) . '">
            <i class="fa fa-arrow-left"></i>
          </a>
        </li>';
}

for ($i = max(1, $curr_page - 2); $i <= min($max_page, $curr_page + 2); $i++) {
